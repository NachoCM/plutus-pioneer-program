...
data GameDatum
  = GameDatum                           -- The game is in-progress
    { player1Move :: ByteString         -- Hash of player 1's nonce plus their choice
    , player2Move :: Maybe GameChoice   -- Player 2's choice, Nothing if they have not played
    }
...
data GameRedeemer
  = Play GameChoice   -- Player 2's choice
  | Reveal            -- Player 1's hash and choice
      ByteString
      GameChoice
  | ClaimFirst
  | ClaimSecond
...
transition game s r = case (stateValue s, stateData s, r) of
    ...
    (v, GameDatum _ (Just c2), Reveal _ c1)
        | lovelaces v == (2 * gStake game)
          && c1 `beats` c2                   -> Just ( Constraints.mustBeSignedBy (gFirst game)                     <>
                                                       Constraints.mustValidateIn (to $ gRevealDeadline game)       <>
                                                       Constraints.mustPayToPubKey (gFirst game) token
                                                     , State Finished mempty
                                                     )
        | lovelaces v == (2 * gStake game)
          && c1 == c2                        -> Just ( Constraints.mustBeSignedBy (gFirst game)                     <>
                                                       Constraints.mustValidateIn (to $ gRevealDeadline game)       <>
                                                       Constraints.mustPayToPubKey (gFirst game) token              <>
...
check bsRock' bsPaper' bsScissors' (GameDatum bs (Just _)) (Reveal nonce c) _ =
  sha2_256 (nonce `concatenate` choice) == bs
  where
    choice :: ByteString
    choice = case c of
      Rock     -> bsRock'
...
firstGame fp = do
    ...
    let game      = Game
        ...
        fpChoice' = fpChoice fp

        -- local helper function
        choiceToBs :: GameChoice -> ByteString
        choiceToBs Rock = bsRock
        choiceToBs Paper = bsPaper
        choiceToBs Scissors = bsScissors

        bs        = sha2_256 $ fpNonce fp `concatenate` choiceToBs fpChoice'
    ...
    case m of
        ...
        -- GetOnChainState :: ... -> Maybe (OnChainState s i, UtxoMap)
        -- type OnChainState s i =
        --    (TypedScriptTxOut (StateMachine s i), TypeScriptTxOutRef (StateMachine s i))
        -- ..so.. o below ::
        --   (OnChainState s i                                                             , UtxoMap)
        --   ((TypedScriptTxOut (StateMachine s i), TypedScriptTxOutRef (StateMachine s i)), UtxoMap)
        Just ((o                                  , _                                     ), _      ) -> case tyTxOutData o of
        -- ..and tyTxOutDate o :: DatumType a
        -- Just ((o, _), _) -> case tyTxOutData o of
            ...
            GameDatum _ (Just spChoice')
              | fpChoice' `beats` spChoice' || fpChoice' == spChoice' -> do
                logInfo @String "second player played and lost or drew"
                void $ mapError' $ runStep client $ Reveal (fpNonce fp) fpChoice'
